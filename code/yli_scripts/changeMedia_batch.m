...
model = setParam(model, 'ub', 'y002100_REV', 1000); % 'water exchange' ;
model = setParam(model, 'ub', 'y001861_REV', 1000); % 'iron(2+) exchange';
model = setParam(model, 'ub', 'y001992_REV', 1000); % 'oxygen exchange';
model = setParam(model, 'ub', 'y002005_REV', 1000); % 'phosphate exchange';
model = setParam(model, 'ub', 'y002060_REV', 1000); % 'sulphate exchange';
model = setParam(model, 'ub', 'y001832_REV', 1000); % 'H+ exchange' ;
model = setParam(model, 'ub', 'y001671_REV', 1000); % Biotin . exchange
model = setParam(model, 'ub', 'y001548_REV', 1000); % pantothenate exchange
model = setParam(model, 'ub', 'y001967_REV', 1000); % Niconitate exchange
model = setParam(model, 'ub', 'y001947_REV', 1000); % Myo-inositol
model = setParam(model, 'ub', 'y002067_REV', 1000); % Thiamin (1+) exchange
model = setParam(model, 'ub', 'y002028_REV', 1000); % Pyridoxine exchange
model = setParam(model, 'ub', 'y001604_REV', 1000); % Aminobenzoic acid
