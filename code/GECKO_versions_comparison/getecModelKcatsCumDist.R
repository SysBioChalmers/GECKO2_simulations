...
kcats_all <- read.csv(file = '../../data/allKineticData.txt', sep = '\t', header = TRUE, stringsAsFactors = FALSE)
kcats_sce <- kcats_all[grep('saccharomyces cerevisiae',kcats_all[,3]),]
kcats_fungi <- kcats_all[grep(';fungi;',kcats_all[,3]),]

kcats_sce <- as.matrix(kcats_sce[,4])
kcats_fungi <- as.matrix(kcats_fungi[,4])
...
plotName <- paste('../../results/Figure_1/Kcats_cumDist_ecYeast.eps',sep='')
...
plotName <- paste('../../results/Figure_1/Kcat_categories_ecYeast.eps',sep='')
plot(p)
